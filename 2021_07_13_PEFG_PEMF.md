...
## Índices de diversidade para as UCs.
...
p3
```
Agora para o calculo iremos fazer uma tabela de localidades por espécie. 
```
...
Primeiro vamos selecionar as tabelas. Vamos fazer duas. Uma indicando a abundância de espécies por dia, denominada de p3 e uma indicando a localidade dessas espécies, denominada p4.
```
...
plot(pool)
```
E nesse a grande diferença entre elas, onde o eix y é a riqueza real amostrada:
`boxplot(specnumber(p3) ~ p4$Locality, col="#efff08", border="black", notch=TRUE)`
