...
```
getwd()
setwd("/home/user/Área de Trabalho/Bio/projeto_publ/2020_altitude/2021_caparaó/R") #Mudança de diretório para pasta do projeto
```
...
```
p2 <- planilhatotal
p2 <- subset(p2, !is.na(Species)) #tirar n/a da ano
...
ggplot(p2, aes(x = Altitude, y = CodSp, colour = RecordedBy)) + 
  ...
  geom_xsideboxplot(aes(fill = RecordedBy),alpha = 0.5, orientation = "y") +
...
```
dicalt <- lm(p2$Altitude ~ p2$RecordedBy, data = p2) 
anova(dicalt) 
```
...
```
fit <- lm(Altitude ~ Locality, data = p2) #some os primeiros de cada grupo
summ(fit)
...
kruskal.test(Altitude~RecordedBy, data = p2)       

g1 <- p2 %>% 
  mutate(RecordedBy = as.factor(RecordedBy)) %>% 
  ggplot() +
  geom_boxplot(aes(y = Altitude, x = RecordedBy)) +
  labs(x = 'RecordedBy', y = 'Altitude', 
       title = 'Boxplot de consumo') + theme_classic()
       
g1 
Fazer com as classes

pairwise.wilcox.test(p3$Altitude,
                          p3$Altitude,
